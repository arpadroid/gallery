...
export const DefaultStory = {
    ...
    playSetup: async (canvasElement, initializeList = _initializeList) => {
        ...
        galleryNode && initializeList && (await DefaultStory.initializeList(galleryNode?.id));
     ...
     */
    render: args => {
        return html` <arpa-gallery ${attrString(args)}>${DefaultStory.renderItemTemplate()}</arpa-gallery> `;
    },
    /**
     * Renders the gallery.
     * @param {import('./gallery.types').GalleryConfigType} args
    ...
    renderStatic: (args, tag = 'arpa-gallery') => {
        return html`
            <${tag} ${attrString(args)}>
                ${DefaultStory.renderItemTemplate()}
                <gallery-item
                    id="gallery-item-1"
...
export const TestDefault = {
    ...
    play: async ({ canvasElement, step }) => {
        const { galleryNode, canvas } = await TestDefault.playSetup(canvasElement);
        /** @type {GalleryItem | null} */
